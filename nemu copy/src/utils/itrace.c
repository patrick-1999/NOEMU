...
void display_inst() {
  ...
  void disassemble(char *str, int size, uint32_t pc, uint8_t *code, int nbyte);
...
void parse_elf(const char *elf_file) {
	if (elf_file == NULL) {
		// Log("elf_file is not given\n");
		printf("elf_file is not given\n");
		return;
	}
