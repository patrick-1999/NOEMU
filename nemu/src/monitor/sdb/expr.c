...
enum {
	...
	TK_PLUS, TK_SUB, TK_MTP, TK_DID, 
	TK_LB, TK_RB, 
	TK_EQ, TK_NEQ, 
	TK_AND,TK_NEG, TK_PTR,
    ...
    for (i = 0; i < NR_REGEX; i ++) {
      if (regexec(&re[i], e + position, 1, &pmatch, 0) == 0 && pmatch.rm_so == 0) {
        ...
        switch (rules[i].token_type) {
          	//actually there is no need for so much switch case
         case TK_NOTYPE : 
            break;
          case TK_PLUS :
          ...
          case TK_REG :
            ...
            new_token.str[substr_len] = '\0';
            printf("new_token.str:%s",new_token.str);
            break;
          default: 
